// ==================== Rhetoric Lens - 知乎主控制器 ====================

/**
 * Rhetoric Lens 知乎编排器
 * 协调所有核心模块和平台适配器
 */
class RhetoricLensZhihu {
  constructor() {
    // 加载所有依赖模块
    ...
    // 平台适配器
    this.platformAdapter = new window.ZhihuAdapter();
   ...
   */
  async init() {
    console.log('[Rhetoric Lens] 🚀 初始化中 (知乎)...');
    ...
    try {
      ...
      console.log(`[Rhetoric Lens] ✅ 已启动 (知乎) | 模型: ${this.config.get('MODEL_NAME')} | 并发: ${this.config.get('MAX_CONCURRENT')}`);
    } catch (error) {
  ...
  scanPage() {
    ...
    console.log(`[Rhetoric Lens] 📊 发现 ${contentItems.length} 条新内容 (知乎)`);
  ...
  async processContentItem(item) {
    const contentType = item.metadata.contentType === 'answer' ? '回答' : '评论';
    console.log(`[Rhetoric Lens] 🎯 准备处理${contentType}: ${item.text.substring(0, 50)}...`);
  ...
  async performAnalysis(item, insertionPoint) {
    const contentType = item.metadata.contentType === 'answer' ? '回答' : '评论';
    console.log(`[Rhetoric Lens] 🔬 开始分析${contentType}: ${item.text.substring(0, 50)}...`);
    ...
    try {
      ...
      if (!result.error && !fromCache) {
        console.log(`[Rhetoric Lens] 💾 保存${contentType}分析结果到数据集`);
      ...
      console.log(`[Rhetoric Lens] ✅ ${contentType} R:${result.rhetoric_score} M:${result.manipulation_score}`);
...
// ==================== 初始化 ====================
(function() {
  console.log('[Rhetoric Lens] 📦 加载知乎 content script...');
  ...
  function init() {
    ...
    // 创建并初始化应用
    const app = new RhetoricLensZhihu();
    app.init();
